...
/**
 * 杂发单头（MisDTO）
 * 单据类型	MiscShipDocType	String	Y
 * 记账期间	SOBAccountPeriod	string	Y	2020-01
 * 业务日期	BusinessDate	Date	Y	默认当前日期
 * 备注	Memo	String
 * 杂发行数据	MisLineDTO	List< MisLineDTO>	Y
 * 公共字段1到段50	PubDescSeg1到PubDescSeg50	string
 * 私有字段字段1到段30	PrivateDescSeg1到PrivateDescSeg30	string
 */
@XmlRootElement(name = "MisDTO")
...
public class MisDTO {
    ...
    @XmlElement(name = "MiscShipDocType")
    @JSONField(name = "MiscShipDocType")
    private String MiscShipDocType;

    @XmlElement(name = "SOBAccountPeriod")
    @JSONField(name = "SOBAccountPeriod")
    ...
    @XmlElement(name = "MisLineDTO")
    @JSONField(name = "MisLineDTO")
    List<MisLineDTO> misLineDTOList = new ArrayList<>();
    ...
    public void setMiscShipDocType(String miscShipDocType) {
        MiscShipDocType = miscShipDocType;
    }
    ...
    public List<MisLineDTO> getMisLineDTOList() {
        return misLineDTOList;
    }

    public void setMisLineDTOList(List<MisLineDTO> misLineDTOList) {
        this.misLineDTOList = misLineDTOList;
    }
