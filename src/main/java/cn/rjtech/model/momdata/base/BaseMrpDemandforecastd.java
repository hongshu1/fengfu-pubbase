package cn.rjtech.model.momdata.base;

import cn.jbolt.core.gen.JBoltField;
import cn.jbolt.core.model.base.JBoltBaseModel;
import com.alibaba.fastjson.annotation.JSONField;
import com.alibaba.fastjson.serializer.ToStringSerializer;

/**
 * 需求计划管理-物料需求预示明细
 * Generated by JBolt, do not modify this file.
 */
@SuppressWarnings("unchecked")
public abstract class BaseMrpDemandforecastd<M extends BaseMrpDemandforecastd<M>> extends JBoltBaseModel<M>{
    /**主键ID*/
    public static final String IAUTOID = "iAutoId";
    /**物料需求预示计算主表ID*/
    public static final String IDEMANDFORECASTMID = "iDemandForecastMid";
    /**年*/
    public static final String IYEAR = "iYear";
    /**月*/
    public static final String IMONTH = "iMonth";
    /**日*/
    public static final String IDATE = "iDate";
    /**供应商ID*/
    public static final String IVENDORID = "iVendorId";
    /**存货ID*/
    public static final String IINVENTORYID = "iInventoryId";
    /**实绩需求数*/
    public static final String IQTY1 = "iQty1";
    /**到货计划数*/
    public static final String IQTY2 = "iQty2";
    /**计划在库*/
    public static final String IQTY3 = "iQty3";
    /**是否锁定;0. 否 1. 是*/
    public static final String ISLOCKED1 = "isLocked1";
    /**是否锁定;0. 否 1. 是*/
    public static final String ISLOCKED2 = "isLocked2";
    /**是否锁定;0. 否 1. 是*/
    public static final String ISLOCKED3 = "isLocked3";
	/**
	 * 主键ID
	 */
	public M setIAutoId(java.lang.Long iAutoId) {
		set("iAutoId", iAutoId);
		return (M)this;
	}

	/**
	 * 主键ID
	 */
	@JBoltField(name="iautoid" ,columnName="iAutoId",type="Long", remark="主键ID", required=true, maxLength=19, fixed=0, order=1)
	@JSONField(name = "iautoid", serializeUsing = ToStringSerializer.class)
	public java.lang.Long getIAutoId() {
		return getLong("iAutoId");
	}

	/**
	 * 物料需求预示计算主表ID
	 */
	public M setIDemandForecastMid(java.lang.Long iDemandForecastMid) {
		set("iDemandForecastMid", iDemandForecastMid);
		return (M)this;
	}

	/**
	 * 物料需求预示计算主表ID
	 */
	@JBoltField(name="idemandforecastmid" ,columnName="iDemandForecastMid",type="Long", remark="物料需求预示计算主表ID", required=true, maxLength=19, fixed=0, order=2)
	@JSONField(name = "idemandforecastmid", serializeUsing = ToStringSerializer.class)
	public java.lang.Long getIDemandForecastMid() {
		return getLong("iDemandForecastMid");
	}

	/**
	 * 年
	 */
	public M setIYear(java.lang.Integer iYear) {
		set("iYear", iYear);
		return (M)this;
	}

	/**
	 * 年
	 */
	@JBoltField(name="iyear" ,columnName="iYear",type="Integer", remark="年", required=true, maxLength=10, fixed=0, order=3)
	@JSONField(name = "iyear")
	public java.lang.Integer getIYear() {
		return getInt("iYear");
	}

	/**
	 * 月
	 */
	public M setIMonth(java.lang.Integer iMonth) {
		set("iMonth", iMonth);
		return (M)this;
	}

	/**
	 * 月
	 */
	@JBoltField(name="imonth" ,columnName="iMonth",type="Integer", remark="月", required=true, maxLength=10, fixed=0, order=4)
	@JSONField(name = "imonth")
	public java.lang.Integer getIMonth() {
		return getInt("iMonth");
	}

	/**
	 * 日
	 */
	public M setIDate(java.lang.Integer iDate) {
		set("iDate", iDate);
		return (M)this;
	}

	/**
	 * 日
	 */
	@JBoltField(name="idate" ,columnName="iDate",type="Integer", remark="日", required=true, maxLength=10, fixed=0, order=5)
	@JSONField(name = "idate")
	public java.lang.Integer getIDate() {
		return getInt("iDate");
	}

	/**
	 * 供应商ID
	 */
	public M setIVendorId(java.lang.Long iVendorId) {
		set("iVendorId", iVendorId);
		return (M)this;
	}

	/**
	 * 供应商ID
	 */
	@JBoltField(name="ivendorid" ,columnName="iVendorId",type="Long", remark="供应商ID", required=true, maxLength=19, fixed=0, order=6)
	@JSONField(name = "ivendorid", serializeUsing = ToStringSerializer.class)
	public java.lang.Long getIVendorId() {
		return getLong("iVendorId");
	}

	/**
	 * 存货ID
	 */
	public M setIInventoryId(java.lang.Long iInventoryId) {
		set("iInventoryId", iInventoryId);
		return (M)this;
	}

	/**
	 * 存货ID
	 */
	@JBoltField(name="iinventoryid" ,columnName="iInventoryId",type="Long", remark="存货ID", required=true, maxLength=19, fixed=0, order=7)
	@JSONField(name = "iinventoryid", serializeUsing = ToStringSerializer.class)
	public java.lang.Long getIInventoryId() {
		return getLong("iInventoryId");
	}

	/**
	 * 实绩需求数
	 */
	public M setIQty1(java.math.BigDecimal iQty1) {
		set("iQty1", iQty1);
		return (M)this;
	}

	/**
	 * 实绩需求数
	 */
	@JBoltField(name="iqty1" ,columnName="iQty1",type="BigDecimal", remark="实绩需求数", required=true, maxLength=24, fixed=6, order=8)
	@JSONField(name = "iqty1")
	public java.math.BigDecimal getIQty1() {
		return getBigDecimal("iQty1");
	}

	/**
	 * 到货计划数
	 */
	public M setIQty2(java.math.BigDecimal iQty2) {
		set("iQty2", iQty2);
		return (M)this;
	}

	/**
	 * 到货计划数
	 */
	@JBoltField(name="iqty2" ,columnName="iQty2",type="BigDecimal", remark="到货计划数", required=true, maxLength=24, fixed=6, order=9)
	@JSONField(name = "iqty2")
	public java.math.BigDecimal getIQty2() {
		return getBigDecimal("iQty2");
	}

	/**
	 * 计划在库
	 */
	public M setIQty3(java.math.BigDecimal iQty3) {
		set("iQty3", iQty3);
		return (M)this;
	}

	/**
	 * 计划在库
	 */
	@JBoltField(name="iqty3" ,columnName="iQty3",type="BigDecimal", remark="计划在库", required=true, maxLength=24, fixed=6, order=10)
	@JSONField(name = "iqty3")
	public java.math.BigDecimal getIQty3() {
		return getBigDecimal("iQty3");
	}

	/**
	 * 是否锁定;0. 否 1. 是
	 */
	public M setIsLocked1(java.lang.Boolean isLocked1) {
		set("isLocked1", isLocked1);
		return (M)this;
	}

	/**
	 * 是否锁定;0. 否 1. 是
	 */
	@JBoltField(name="islocked1" ,columnName="isLocked1",type="Boolean", remark="是否锁定;0. 否 1. 是", required=true, maxLength=1, fixed=0, order=11)
	@JSONField(name = "islocked1")
	public java.lang.Boolean getIsLocked1() {
		return getBoolean("isLocked1");
	}

	/**
	 * 是否锁定;0. 否 1. 是
	 */
	public M setIsLocked2(java.lang.Boolean isLocked2) {
		set("isLocked2", isLocked2);
		return (M)this;
	}

	/**
	 * 是否锁定;0. 否 1. 是
	 */
	@JBoltField(name="islocked2" ,columnName="isLocked2",type="Boolean", remark="是否锁定;0. 否 1. 是", required=true, maxLength=1, fixed=0, order=12)
	@JSONField(name = "islocked2")
	public java.lang.Boolean getIsLocked2() {
		return getBoolean("isLocked2");
	}

	/**
	 * 是否锁定;0. 否 1. 是
	 */
	public M setIsLocked3(java.lang.Boolean isLocked3) {
		set("isLocked3", isLocked3);
		return (M)this;
	}

	/**
	 * 是否锁定;0. 否 1. 是
	 */
	@JBoltField(name="islocked3" ,columnName="isLocked3",type="Boolean", remark="是否锁定;0. 否 1. 是", required=true, maxLength=1, fixed=0, order=13)
	@JSONField(name = "islocked3")
	public java.lang.Boolean getIsLocked3() {
		return getBoolean("isLocked3");
	}

}

