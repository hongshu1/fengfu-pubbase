package cn.rjtech.model.momdata.base;
import cn.jbolt.core.model.base.JBoltBaseModel;
import cn.jbolt.core.gen.JBoltField;
import com.alibaba.fastjson.annotation.JSONField;
import com.alibaba.fastjson.serializer.ToStringSerializer;

/**
 * 生产计划-月周生产计划数量
 * Generated by JBolt, do not modify this file.
 */
@SuppressWarnings({"serial", "unchecked"})
public abstract class BaseApsWeekscheduledQty<M extends BaseApsWeekscheduledQty<M>> extends JBoltBaseModel<M>{
    /**主键ID*/
    public static final String IAUTOID = "iAutoId";
    /**周计划明细ID*/
    public static final String IWEEKSCHEDULEDID = "iWeekScheduleDid";
    /**年*/
    public static final String IYEAR = "iYear";
    /**月*/
    public static final String IMONTH = "iMonth";
    /**日*/
    public static final String IDATE = "iDate";
    /**计划使用*/
    public static final String IQTY1 = "iQty1";
    /**计划/1S*/
    public static final String IQTY2 = "iQty2";
    /**计划/2S*/
    public static final String IQTY3 = "iQty3";
    /**计划/3S*/
    public static final String IQTY4 = "iQty4";
    /**计划在库*/
    public static final String IQTY5 = "iQty5";
    /**在库天数*/
    public static final String IQTY6 = "iQty6";
    /**是否锁定：0. 否 1. 是*/
    public static final String ISLOCKED = "isLocked";
    /**是否周末: 0. 否 1. 是*/
    public static final String ISWEEKEND = "isWeekend";
    /**是否人工修改：0. 否 1. 是*/
    public static final String ISMODIFIED = "isModified";
	/**
	 * 主键ID
	 */
	public M setIAutoId(java.lang.Long iAutoId) {
		set("iAutoId", iAutoId);
		return (M)this;
	}

	/**
	 * 主键ID
	 */
	@JBoltField(name="iautoid" ,columnName="iAutoId",type="Long", remark="主键ID", required=true, maxLength=19, fixed=0, order=1)
	@JSONField(name = "iautoid", serializeUsing = ToStringSerializer.class)
	public java.lang.Long getIAutoId() {
		return getLong("iAutoId");
	}

	/**
	 * 周计划明细ID
	 */
	public M setIWeekScheduleDid(java.lang.Long iWeekScheduleDid) {
		set("iWeekScheduleDid", iWeekScheduleDid);
		return (M)this;
	}

	/**
	 * 周计划明细ID
	 */
	@JBoltField(name="iweekscheduledid" ,columnName="iWeekScheduleDid",type="Long", remark="周计划明细ID", required=true, maxLength=19, fixed=0, order=2)
	@JSONField(name = "iweekscheduledid", serializeUsing = ToStringSerializer.class)
	public java.lang.Long getIWeekScheduleDid() {
		return getLong("iWeekScheduleDid");
	}

	/**
	 * 年
	 */
	public M setIYear(java.lang.Integer iYear) {
		set("iYear", iYear);
		return (M)this;
	}

	/**
	 * 年
	 */
	@JBoltField(name="iyear" ,columnName="iYear",type="Integer", remark="年", required=true, maxLength=10, fixed=0, order=3)
	@JSONField(name = "iyear")
	public java.lang.Integer getIYear() {
		return getInt("iYear");
	}

	/**
	 * 月
	 */
	public M setIMonth(java.lang.Integer iMonth) {
		set("iMonth", iMonth);
		return (M)this;
	}

	/**
	 * 月
	 */
	@JBoltField(name="imonth" ,columnName="iMonth",type="Integer", remark="月", required=true, maxLength=10, fixed=0, order=4)
	@JSONField(name = "imonth")
	public java.lang.Integer getIMonth() {
		return getInt("iMonth");
	}

	/**
	 * 日
	 */
	public M setIDate(java.lang.Integer iDate) {
		set("iDate", iDate);
		return (M)this;
	}

	/**
	 * 日
	 */
	@JBoltField(name="idate" ,columnName="iDate",type="Integer", remark="日", required=true, maxLength=10, fixed=0, order=5)
	@JSONField(name = "idate")
	public java.lang.Integer getIDate() {
		return getInt("iDate");
	}

	/**
	 * 计划使用
	 */
	public M setIQty1(java.math.BigDecimal iQty1) {
		set("iQty1", iQty1);
		return (M)this;
	}

	/**
	 * 计划使用
	 */
	@JBoltField(name="iqty1" ,columnName="iQty1",type="BigDecimal", remark="计划使用", required=false, maxLength=18, fixed=2, order=6)
	@JSONField(name = "iqty1")
	public java.math.BigDecimal getIQty1() {
		return getBigDecimal("iQty1");
	}

	/**
	 * 计划/1S
	 */
	public M setIQty2(java.math.BigDecimal iQty2) {
		set("iQty2", iQty2);
		return (M)this;
	}

	/**
	 * 计划/1S
	 */
	@JBoltField(name="iqty2" ,columnName="iQty2",type="BigDecimal", remark="计划/1S", required=false, maxLength=18, fixed=2, order=7)
	@JSONField(name = "iqty2")
	public java.math.BigDecimal getIQty2() {
		return getBigDecimal("iQty2");
	}

	/**
	 * 计划/2S
	 */
	public M setIQty3(java.math.BigDecimal iQty3) {
		set("iQty3", iQty3);
		return (M)this;
	}

	/**
	 * 计划/2S
	 */
	@JBoltField(name="iqty3" ,columnName="iQty3",type="BigDecimal", remark="计划/2S", required=false, maxLength=18, fixed=2, order=8)
	@JSONField(name = "iqty3")
	public java.math.BigDecimal getIQty3() {
		return getBigDecimal("iQty3");
	}

	/**
	 * 计划/3S
	 */
	public M setIQty4(java.math.BigDecimal iQty4) {
		set("iQty4", iQty4);
		return (M)this;
	}

	/**
	 * 计划/3S
	 */
	@JBoltField(name="iqty4" ,columnName="iQty4",type="BigDecimal", remark="计划/3S", required=false, maxLength=18, fixed=2, order=9)
	@JSONField(name = "iqty4")
	public java.math.BigDecimal getIQty4() {
		return getBigDecimal("iQty4");
	}

	/**
	 * 计划在库
	 */
	public M setIQty5(java.math.BigDecimal iQty5) {
		set("iQty5", iQty5);
		return (M)this;
	}

	/**
	 * 计划在库
	 */
	@JBoltField(name="iqty5" ,columnName="iQty5",type="BigDecimal", remark="计划在库", required=false, maxLength=18, fixed=2, order=10)
	@JSONField(name = "iqty5")
	public java.math.BigDecimal getIQty5() {
		return getBigDecimal("iQty5");
	}

	/**
	 * 在库天数
	 */
	public M setIQty6(java.math.BigDecimal iQty6) {
		set("iQty6", iQty6);
		return (M)this;
	}

	/**
	 * 在库天数
	 */
	@JBoltField(name="iqty6" ,columnName="iQty6",type="BigDecimal", remark="在库天数", required=false, maxLength=18, fixed=2, order=11)
	@JSONField(name = "iqty6")
	public java.math.BigDecimal getIQty6() {
		return getBigDecimal("iQty6");
	}

	/**
	 * 是否锁定：0. 否 1. 是
	 */
	public M setIsLocked(java.lang.Boolean isLocked) {
		set("isLocked", isLocked);
		return (M)this;
	}

	/**
	 * 是否锁定：0. 否 1. 是
	 */
	@JBoltField(name="islocked" ,columnName="isLocked",type="Boolean", remark="是否锁定：0. 否 1. 是", required=true, maxLength=1, fixed=0, order=12)
	@JSONField(name = "islocked")
	public java.lang.Boolean getIsLocked() {
		return getBoolean("isLocked");
	}

	/**
	 * 是否周末: 0. 否 1. 是
	 */
	public M setIsWeekend(java.lang.Boolean isWeekend) {
		set("isWeekend", isWeekend);
		return (M)this;
	}

	/**
	 * 是否周末: 0. 否 1. 是
	 */
	@JBoltField(name="isweekend" ,columnName="isWeekend",type="Boolean", remark="是否周末: 0. 否 1. 是", required=true, maxLength=1, fixed=0, order=13)
	@JSONField(name = "isweekend")
	public java.lang.Boolean getIsWeekend() {
		return getBoolean("isWeekend");
	}

	/**
	 * 是否人工修改：0. 否 1. 是
	 */
	public M setIsModified(java.lang.Boolean isModified) {
		set("isModified", isModified);
		return (M)this;
	}

	/**
	 * 是否人工修改：0. 否 1. 是
	 */
	@JBoltField(name="ismodified" ,columnName="isModified",type="Boolean", remark="是否人工修改：0. 否 1. 是", required=true, maxLength=1, fixed=0, order=14)
	@JSONField(name = "ismodified")
	public java.lang.Boolean getIsModified() {
		return getBoolean("isModified");
	}

}

