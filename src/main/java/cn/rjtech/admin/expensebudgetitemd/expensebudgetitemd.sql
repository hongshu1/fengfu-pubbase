...
from #(getMesDbName()).dbo.PL_Expense_Budget_Item ebi
    ...
    left join #(getMesDbName()).dbo.bas_subjectm bsh on bsh.iAutoId = ebi.iHighestSubjectId
    left join #(getMesDbName()).dbo.bas_subjectm bsl on bsl.iAutoId = ebi.iLowestSubjectId
where 1=1
    #if(ibudgetyear)
