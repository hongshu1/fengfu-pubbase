...
#if(cinvcode)
  and t1.cInvCode like concat('%',#para(cinvcode),'%')
#end
#if(cinvname)
  and t1.cInvName like concat('%',#para(cinvname),'%')
#end
#if(cbarcode)
  and t2.cbarcode like concat('%',#para(cbarcode),'%')
#end
  order by t1.dUpdateTime desc
#end

#sql("findShiWuByCSourceId")
...
where 1=1 and log.ProcessType = 'BarcodeMerge'
#if(csourceid)
...
SELECT log.*,inv.invname,inv.invstd,inv.unitname,
       t3.iInventoryUomId1,t3.cInvCode1,t3.cInvName1,
...
WHERE log.ProcessType = 'BarcodeMerge'
#if(organizeCode)
