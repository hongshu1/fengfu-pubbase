...
public class SysProductindetailService extends BaseService<SysProductindetail> {
	...
	public Ret save(SysProductindetail sysProductindetail) {
		...
		//if(existsName(sysProductindetail.getName())) {return fail(JBoltMsg.DATA_SAME_NAME_EXIST);}
		boolean success=sysProductindetail.save();
		if(success) {
			//添加日志
			//addSaveSystemLog(sysProductindetail.getAutoID(), JBoltUserKit.getUserId(), sysProductindetail.getName());
		}
		return ret(success);
	}
	...
	public Ret update(SysProductindetail sysProductindetail) {
		...
		if(dbSysProductindetail==null) {return fail(JBoltMsg.DATA_NOT_EXIST);}
		//if(existsName(sysProductindetail.getName(), sysProductindetail.getAutoID())) {return fail(JBoltMsg.DATA_SAME_NAME_EXIST);}
		boolean success=sysProductindetail.update();
		if(success) {
			//添加日志
			//addUpdateSystemLog(sysProductindetail.getAutoID(), JBoltUserKit.getUserId(), sysProductindetail.getName());
		}
		return ret(success);
	}
	...
	protected String afterDelete(SysProductindetail sysProductindetail, Kv kv) {
		//addDeleteSystemLog(sysProductindetail.getAutoID(), JBoltUserKit.getUserId(),sysProductindetail.getName());
		return null;
	}
	...
	protected String afterToggleBoolean(SysProductindetail sysProductindetail, String column, Kv kv) {
		//addUpdateSystemLog(sysProductindetail.getAutoID(), JBoltUserKit.getUserId(), sysProductindetail.getName(),"的字段["+column+"]值:"+sysProductindetail.get(column));
		/**
		switch(column){
		    case "IsDeleted":
		        break;
