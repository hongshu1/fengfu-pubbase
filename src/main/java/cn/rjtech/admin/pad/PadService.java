...
public class PadService extends BaseService<Pad> {
	...
	public Ret save(Pad pad) {
		...
		if(exists("cPadCode",pad.getCPadCode())) {return fail(JBoltMsg.DATA_SAME_SN_EXIST);}
		if(exists("cMac",pad.getCMac())) {return fail("Mac地址已存在");}
	...
	public Ret update(Pad pad) {
		...
		if(dbPad==null) {return fail(JBoltMsg.DATA_NOT_EXIST);}
