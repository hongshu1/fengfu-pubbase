...
select p.*,d.cDepName,
       (select top 1 i.cInvName from Inventory i where i.cInvCode=p.cInvCode ) as cInvName
from #(getMesDbName()).dbo.Bas_Project p
    left join  Department  d on d.cDepCode=p.cdepcode
where 1=1
    #if(cproject)
...
#sql("u8DepartmentList")
select  cDepCode,cDepName,cDepNameEn
from Department
where iDepGrade = 1
   #if(cDepcode)
