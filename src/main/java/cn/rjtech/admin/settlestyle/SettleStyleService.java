...
public class SettleStyleService extends BaseService<SettleStyle> {
    ...
    public Page<Record> paginateAdminDatas(int pageNumber, int pageSize, Kv para) {
        ...
        for (Record row : pageList.getList()) {
            row.set("issbilltypedesc", JBoltDictionaryCache.me.getNameBySn(DictionaryTypeKey.iSSBillType.name(), row.getStr("issbilltype")));
        }
        return pageList;
    }
    ...
    public Ret save(SettleStyle settleStyle) {
        ...
        Date now = new Date();
        settleStyle.setICreateBy(loginUser.getId()).setCCreateName(loginUser.getName()).setDCreateTime(now).setIUpdateBy(loginUser.getId()).setCUpdateName(loginUser.getName()).setDUpdateTime(now).setIsDeleted(false);
    ...
    public Ret update(SettleStyle settleStyle) {
        ...
        if (dbSettleStyle == null) {
            return fail(JBoltMsg.DATA_NOT_EXIST);
        }
        User loginUser = JBoltUserKit.getUser();
        Date now = new Date();
        settleStyle.setIUpdateBy(loginUser.getId()).setCUpdateName(loginUser.getName()).setDUpdateTime(now);
