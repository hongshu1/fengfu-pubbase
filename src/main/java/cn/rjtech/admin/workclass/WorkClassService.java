...
//	private PersonService personService;
	...
	public Ret update(Workclass workclass) {
		...
		if(dbWorkclass==null) {return fail(JBoltMsg.DATA_NOT_EXIST);}
		//if(existsName(workclass.getName(), workclass.getIautoid())) {return fail(JBoltMsg.DATA_SAME_NAME_EXIST);}
		...
		if(success) {
			//添加日志
			//addUpdateSystemLog(workclass.getIautoid(), JBoltUserKit.getUserId(), workclass.getName());
		}
		return ret(success);
	}
		...
		return JBoltExcel
			...
			.setSheets(
				JBoltExcelSheet.create("工种档案导入模板")
					//设置列映射 顺序 标题名称 不处理别名
					.setHeaders(2, false,
						JBoltExcelHeader.create("工种编码", 15),
						JBoltExcelHeader.create("工种名称", 15),
						JBoltExcelHeader.create("工种等级", 15),
						JBoltExcelHeader.create("工种薪酬", 15),
