...
public class SysOtherindetailService extends BaseService<SysOtherindetail> {
	...
	public Ret save(SysOtherindetail sysOtherindetail) {
		...
		//if(existsName(sysOtherindetail.getName())) {return fail(JBoltMsg.DATA_SAME_NAME_EXIST);}
		boolean success=sysOtherindetail.save();
		if(success) {
			//添加日志
			//addSaveSystemLog(sysOtherindetail.getAutoID(), JBoltUserKit.getUserId(), sysOtherindetail.getName());
		}
		return ret(success);
	}
	...
	public Ret update(SysOtherindetail sysOtherindetail) {
		...
		if(dbSysOtherindetail==null) {return fail(JBoltMsg.DATA_NOT_EXIST);}
		//if(existsName(sysOtherindetail.getName(), sysOtherindetail.getAutoID())) {return fail(JBoltMsg.DATA_SAME_NAME_EXIST);}
		boolean success=sysOtherindetail.update();
		if(success) {
			//添加日志
			//addUpdateSystemLog(sysOtherindetail.getAutoID(), JBoltUserKit.getUserId(), sysOtherindetail.getName());
		}
		return ret(success);
	}
	...
	protected String afterDelete(SysOtherindetail sysOtherindetail, Kv kv) {
		//addDeleteSystemLog(sysOtherindetail.getAutoID(), JBoltUserKit.getUserId(),sysOtherindetail.getName());
		return null;
	}
