...
public class WorkshiftmService extends BaseService<Workshiftm> {
	...
	public Ret update(Workshiftm workshiftm) {
		...
		if(dbWorkshiftm==null) {return fail(JBoltMsg.DATA_NOT_EXIST);}
	...
	public Ret importExcelData(File file) {
		...
		if(models.size()>0){
			...
			for(Workshiftm p:models){
				...
				ValidationUtils.notNull(p.getStr("dendtime"), "出勤结束时间为空！");
				ValidationUtils.isTrue(findFirst(Okv.by("cWorkShiftCode", p.getStr("cworkshiftcode")).set("isDeleted", 0), "iautoid", "asc")==null, p.getStr("cworkshiftcode")+"编码重复");

				workshiftmsSet.add(Kv.by("cworkshiftcode", p.getStr("cworkshiftcode")).set("cworkshiftname",p.getStr("cworkshiftname")).set("dstarttime", p.getStr("dstarttime")).set("dendtime", p.getStr("dendtime")));
			}
			...
			for(Kv kv : workshiftmsSet){
				...
				for(Workshiftm p:models){
					if(isOk(p.getStr("itype")) && p.getStr("cworkshiftcode").equals(workshiftm.getCworkshiftcode())){
						...
						//两个时间相差分钟
						long iminute= (new DateTime("2022-11-01 "+workshiftd.getDendtime()+":00").getTime()-new DateTime("2022-11-01 "+workshiftd.getDstarttime()+":00").getTime())/(1000*60);
						workshiftd.setIminute(Integer.valueOf(String.valueOf(iminute)));
