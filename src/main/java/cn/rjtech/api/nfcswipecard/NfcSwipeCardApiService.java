...
public class NfcSwipeCardApiService extends JBoltApiBaseService {
   ...
   * @return
   */
  public Record nfcswipecard(String deviceaddressstr,
                             String cardusercode,
                             String cardcode,
                             String cpadname) {
    ...
    //根据mac地址获取当前平板对应的产线
    Record deviceaddress = padService.getPadWorkRegionByCmac(deviceaddressstr);
    boolean bol = true;
    boolean bol1 = true;
    Record records = personEquipmentService.getPersonequipmentByCpsnnumId(Kv.by("iautoid", deviceaddress.getLong("iworkregionmid"))
        .set("cpsnnum", cardcode));
    if (records != null) {
      bol = false;
    ...
    List<Record> modocByUserAnRegionids = moDocService.getMoworkshiftdByUserAnRegionid(deviceaddress.getLong("iworkregionmid"), cardcode);
    bol1 = modocByUserAnRegionids.size() < 0 ? false : true;
    if (!bol1) {
      //查找工单工艺人员配置
      List<Record> usercodes = moDocService.getMoroutingconfigpersonByUserAnRegionid(deviceaddress.getLong("iworkregionmid"), cardcode);
