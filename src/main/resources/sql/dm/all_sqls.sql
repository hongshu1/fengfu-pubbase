...
#define order()
 ...
 #for(col:orderColumns)#(for.first?" order by ":"")#if(col.contains("(")&&col.contains(")"))#(col) #else "#(col)" #end #if(orderTypes!=null&&orderTypes.length>0)#(orderTypes[for.index])#end #(for.last?"":",")#end
#end
#end
