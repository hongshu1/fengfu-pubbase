...
UE.Editor.prototype.getActionUrl = function(action) {
    ...
    if (action == 'uploadimage' || action == 'uploadscrawl') {
        return '/admin/neditor/upload/image/';
    } else if (action == 'uploadvideo') {
    	return '/admin/neditor/upload/video/';
    } else if (action == 'catchimage') {
        return '/admin/neditor/upload/catchImage/';
    } else {
...
window.UEDITOR_CONFIG['scrawlUploadService'] = function(context, editor) {
    return scrawlUploadService = {
    		uploadScrawForIE: function(fileName, data, success, fail) {
                ...
                $.ajax({
                    ...
                    var jboltres = typeof(res)=='object'?res:JSON.parse(res);
                    
                    /* 上传接口返回的response成功状态条件 (比如: res.code == 200) */
                    jboltres.responseSuccess = res.code == 200;

                    /* 指定上传接口返回的response中涂鸦图片路径的字段，默认为 url 
                     * 如果涂鸦图片路径字段不是res的属性，可以写成 对象.属性 的方式，例如：data.url
                     */
                    jboltres.scrawlSrcField = 'url';

                    /* 上传成功 */
                    success.call(context, jboltres);
                }).fail(function(err) {
                    /* 上传失败 */
