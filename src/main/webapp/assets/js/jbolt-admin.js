...
//		"fileinput":{"js":['assets/plugins/bootstrap-fileinput/js/plugins/canvas-to-blob.min.js','assets/plugins/bootstrap-fileinput/js/plugins/sortable.min.js','assets/plugins/bootstrap-fileinput/js/plugins/piexif.min.js','assets/plugins/bootstrap-fileinput/js/plugins/purify.min.js','assets/plugins/bootstrap-fileinput/js/fileinput.min.js','assets/plugins/bootstrap-fileinput/themes/fa/theme.min.js','assets/plugins/bootstrap-fileinput/themes/explorer-fa/theme.min.js','assets/plugins/bootstrap-fileinput/js/locales/zh.js'],"css":['assets/plugins/bootstrap-fileinput/css/fileinput.min.css','assets/plugins/bootstrap-fileinput/themes/explorer-fa/theme.min.css']},
		...
		"rangerslider":{"js":['assets/plugins/rangerslider/ion.rangeSlider.min.js'],css:['assets/plugins/rangerslider/ion.rangeSlider.min.css']},
		"hiprint":{"js":['assets/plugins/hiprint/polyfill.min.js','assets/plugins/hiprint/hiprint.bundle.js','assets/plugins/hiprint/plugins/JsBarcode.all.min.js','assets/plugins/hiprint/plugins/qrcode.js','assets/plugins/hiprint/plugins/jquery.hiwprint.js'],css:['assets/plugins/hiprint/css/hiprint.css','assets/plugins/hiprint/css/print-lock.css',{url:"assets/plugins/hiprint/css/print-lock.css",media:"print"}]},
...
var JBoltBaiduMapUtil={
	init:function(apiVersion,ak,callback){
		window.JBoltBaiduMapLoadedCallback=function(){
		 ...
		 script.src = '//api.map.baidu.com/api?type=webgl&v=1.0&ak='+ak+'&callback=JBoltBaiduMapLoadedCallback';
				...
				if(isOk(inputId)){
					...
					if(hiddenInputId){
						if(hiddenInputId.indexOf(",")!=-1){
							...
							$("#"+hiddenInputId).val(value).change();
						}
					}
					if(jsonData){
		...
		showJBoltInputLayer:function(input){
			...
			if(!input_layer){
				...
				if(loadType=="html"){
					...
					//var inputId=input.attr("id");
					var treeBox;
					var openType=input.data("open-type");
					openType=openType?("data-open-type=\""+openType+"\""):'';
					...
					if(input.data("filter-handler")){
						treeBox=$('<div  data-onlyattrfilter="'+onlyattrfilter+'" data-onlyleaf="'+onlyLeaf+'" data-onlytype="'+onlyType+'" data-textasvalue="'+textasvalue+'" data-jboltinput="'+input.attr("id")+'" '+openType+' data-search-by-jboltinput="true" data-jstree '+(hasCheckbox?" data-checkbox='true' ":'')+' data-read-url="'+url+'"  data-change-handler="jstreeSetJBoltInputValueHandler"> </div>');
					}else{
...
//rangerSlider组件
var RangerSliderUtil={
		init:function(parentEle){
			var parent=getRealParentJqueryObject(parentEle);
		 	if(!isOk(parent)){return false;}
		 	var inputs=parent.find("input[data-rangerslider]");
		 	if(!isOk(inputs)){return false;}
		 	this.initInputs(inputs);
		},initInputs:function(inputs){
			if(!isOk(inputs)){return false;}
			var that=this;
			loadJBoltPlugin(['rangerslider'], function(){
				var len=inputs.length;
				for(var i=0;i<len;i++){
					that.initInput(inputs.eq(i),false);
				}
...
var CityPickerUtil={
	...
	init:function(parentEle){
		...
		if(!setvalueto){
			LayerMsgBox.alert("请设置cityPicker的 data-setvalueto属性 格式 id:name:valueAttr",2);
			return false;
		}
		...
		if(setvalueto.indexOf(";")!=-1){
			...
			for(var i in arr){
				tempArr=arr[i].split(":");
				if(!isOk(tempArr)||tempArr.length!=3){
					LayerMsgBox.alert("请正确设置cityPicker的 data-setvalueto属性 格式 id:name:valueAttr",2);
					return false;
				}
		...
		var pickerOptions={
	             ...
	             onSelect: function ( code, name, all ) {
	            	 that.selectCity(code,name,all,setValueArr);
	            	 if(selectHandler){
		...
		var selectedCallback=cp.data("selectedCallback");
		if(selectedCallback){
	...
	initHiddenInput:function(cp,setValueArr){
		var hiddeninputId,hiddeninputName,hiddenInput;
	...
	selectCity:function(code,name,all,setValueArr){
		var hiddeninputId,hiddeninputName,valueAttr,inputValue,hiddenInput;
		for(var i in setValueArr){
			hiddeninputId=setValueArr[i][0];
			hiddeninputName=setValueArr[i][1];
				 ...
				 tree.jstree(true).get_all_checked = function(full,withroot,attr,onlyleaf,onlytype,dataAttrFilterFunc) {
					 ...
					 for(var i in datas){
						 ...
						 if((onlyleaf && this.is_checked(i) && this.is_leaf(i)) ||(!onlyleaf && (this.is_undetermined(i)||this.is_checked(i)))){
							 tempId = attr?(tempData[attr]?tempData[attr]:tempData.id):tempData.id;
							 if(!withroot && (tempId=='#' || tempId == '0')){
								 continue;
							 }
							 if(isOk(onlytype)){
...
function processAutocompleteItem(data,column_attr,align){
	...
	if(column_attr.indexOf(",")==-1){
		...
		return "<span style='width:100%;text-align:"+align+"'>"+text+"</span>";
	}
	...
	for(var i in attrs){
		...
		text=text+"<span style='width:"+width+";text-align:"+textAlign+"'>"+t+"</span>";
	}
	return text;
}
/**
...
var JBoltLayerUtil={
		...
		},close:function(dontShowConfirm){
			var that=this;
			var existLayer=$("#jbolt_layer");
			if(existLayer&&existLayer.length==1){
				var triggerid=existLayer.data("triggerid");
...
var SwitchBtnUtil={
		...
		set:function(name,value,parentEle){
			 var parent=getRealParentJqueryObject(parentEle);
			 if(!isOk(parent)){return;}
			 var img=parent.find("img[data-switchbtn][data-name='"+name+"']");
			 if(!isOk(img)){return;}
			 value=value+'';
...
var ImgUploadUtil={
		  ...
		  changeFiles:function(uploder,fileInput,fileDatas){
				...
				if(handler){
					...
					if(imgbox){
						var imgs=$("#"+imgbox+" img");
						if(isOk(imgs)&&(imgs.length>=limit||(imgs.length+fileDatas.length>limit))){
								...
								if(ucallback){
									var execa_handler=eval(ucallback);
									  if(execa_handler&&typeof(execa_handler)=="function"){
										  execa_handler(type,fileInput,res);
									}
								}
							});
...
function removeUploadFileBoxLi(ele){
	...
	if(isOk(btn)){
		...
		if(removeUrl && removeUrl.indexOf("file=[file]")!=-1){
			var file = li.data("remove-file");
			if(!file){
				LayerMsgBox.alert("data-remove-url中存在file=[file]，需要文件上携带data-path或者data-id",2);
				return;
			}
 ...
 * @returns
 */
function processImgUploadCallback(res,hiddeninput,isMultiple,imgbox){
	var datas=res.data;
	var tmpImgUrl;
	var path;
		...
		if(isArray(datas)){
			if(typeof(datas[0])=="string"){
				...
				$.each(datas,function(i,fdata){
					path.push(fdata.url||fdata.localUrl);
					tmpImgUrl.push(real_image(fdata.url||fdata.localUrl))
				});
			}
		}else{
	...
	if(isMultiple&&imgbox){
		var imgstyle=$("#"+imgbox).data("imgstyle")||"";
		if(isArray(tmpImgUrl)){
			$.each(tmpImgUrl,function(i,bimgurl){
				$("#"+imgbox).append("<li data-imgbox='"+imgbox+"' data-hiddeninput='"+hiddeninputId+"'><img data-title='阿斯达所多撒' data-photobtn data-album='"+hiddeninput+"' style='"+imgstyle+"' data-path='"+path[i]+"' src='"+bimgurl+"'/><div class='optbox' ><i title='查看大图' class='fa fa-eye' onclick='lookUploadImgPhoto(this)'></i><i title='删除' onclick='removeUploadImgBoxLi(this)' class='fa fa-trash'></i><i onclick='imgGotoLeft(this)' class='fa fa-arrow-left' title='左移'></i><i title='右移' onclick='imgGotoRight(this)' class='fa fa-arrow-right'></i></div></li>").show();
			});
			if(!hvalue){
				hvalue=path.join(",");
			}else{
			...
			$("#"+imgbox).append("<li data-imgbox='"+imgbox+"' data-hiddeninput='"+hiddeninputId+"'><img data-photobtn data-album='"+hiddeninput+"' style='"+imgstyle+"' data-path='"+path+"' src='"+tmpImgUrl+"'/><div class='optbox' ><i title='查看大图' class='fa fa-eye' onclick='lookUploadImgPhoto(this)'></i><i title='删除' onclick='removeUploadImgBoxLi(this)' class='fa fa-trash'></i><i onclick='imgGotoLeft(this)' class='fa fa-arrow-left' title='左移'></i><i title='右移' onclick='imgGotoRight(this)' class='fa fa-arrow-right'></i></div></li>").show();
			if(!hvalue){
				hvalue=path;
			}else{
				hvalue=hvalue+","+path;
...
function processFileUploadCallback(res,hiddeninput,filenameInput,isMultiple,fileboxId){
		...
		if(isArray(datas)){
			if(typeof(datas[0])=="string"){
				...
				$.each(datas,function(i,fdata){
					...
					tmpFileNames.push(fdata.fileName?fdata.fileName:getFileName(fdata.localUrl||fdata.url||fdata.fileUrl));
				});
			}
		}else{
	...
	if(fileboxId){
		var filebox = $("#"+fileboxId);
		var tpl = filebox.data("tpl");
		var tplContent;
		if(tpl){
			var tplTextarea = $("#"+tpl);
			if(isOk(tplTextarea)){
				tplContent = tplTextarea.val();
			}
		}
		...
		if(isArray(tmpFilePaths)){
			if(tplContent){
				...
				$.each(tmpFilePaths,function(i,bfileurl){
					datas.push({path:path[i],url:bfileurl,fileName:tmpFileNames[i],fileSuffix:getFileExtension(tmpFileNames[i]),isImg:isImg(tmpFileNames[i])});
				});
				...
				if(tpl_html){
					...
					$.each(tmpFilePaths,function(i,bfileurl){
						filebox.append("<li data-filebox='"+fileboxId+"' data-hiddeninput='"+hiddeninputId+"'><a style='"+filestyle+"' target='_blank' data-path='"+path[i]+"' href='"+bfileurl+"'>"+tmpFileNames[i]+"</a><i title='删除' onclick='removeUploadFileBoxLi(this)' class='fa fa-trash text-danger ml-3'></i></li>").show();
					});
				}
			}else{
				$.each(tmpFilePaths,function(i,bfileurl){
			...
			var datas = [];
			datas.push({path:path,url:tmpFilePaths,fileName:tmpFileNames,fileSuffix:getFileExtension(tmpFileNames),isImg:isImg(tmpFileNames)});
			var tpl_html = "<div>"+juicer(tplContent,{fileboxId:fileboxId,hiddeninputId:hiddeninputId,datas:datas})+"</div>";
			if(tpl_html){
				var tplHtmlObj = $(tpl_html);
				var fileObjs = tplHtmlObj.find(".file");
				...
				filebox.append("<li data-filebox='"+fileboxId+"' data-hiddeninput='"+hiddeninputId+"'><a style='"+filestyle+"' target='_blank' data-path='"+path+"' href='"+tmpFilePaths+"'>"+tmpFileNames+"</a><i title='删除' onclick='removeUploadFileBoxLi(this)' class='fa fa-trash text-danger ml-3'></i></li>").show();
			}
			if(hiddeninput){
				if(!hvalue){
					hvalue=path;
 ...
 * @returns
 */
function uploadFileToQiniu(uploader,type,fileDatas,hiddeninput,filenameInput,sizeinput,isMultiple,imgbox,loading,callback){
		...
		if(res.data){
			...
			_uploadFileToQiniu(uploader,type,fileDatas,loading,res.data,isMultiple,function(){
        		...
        		if((type=="img")&&hiddeninput&&isOk(qiniuFiles)){
        			processImgUploadCallback({data:qiniuFiles},hiddeninput,isMultiple,imgbox);
...
function _uploadFileToQiniu(uploader,type,fileDatas,loading,qiniuUploadPara,isMultiple,callback){
	...
	LayerMsgBox.loading(loading?loading:"上传中",10000);
	
	var fileKey = uploader.data("file-key");
	if(!fileKey){
		fileKey = date(new Date(),"yyyy_MM_dd_"+randomId());
	}else{
		fileKey = fileKey.replaceAll("[dateTime]",date(new Date(),"yyyy_MM_dd_HH_mm_ss"));
		fileKey = fileKey.replaceAll("[date]",date(new Date(),"yyyy_MM_dd"));
 ...
 * @returns
 */
function uploadFile(type,url,name,fileDatas,hiddeninput,filenameInput,sizeinput,isMultiple,imgbox,loading,callback){
	if(!fileDatas){return false;}
	if(isMultiple&&fileDatas.length==0){return false;}
	if(!isMultiple&&!fileDatas.size){return false;}
	LayerMsgBox.loading(loading?loading:"上传中",10000);
	 ...
	 if(isMultiple){
	    ...
	    var timeout=60000;
	    if(isOk(hiddeninput)){
	    	timeout=hiddeninput.data("timeout")||60000;
	    }
	    $.ajax({
	        type:"POST",
	        url: url,
	        ...
	        success:function (res) {
	        	if(res.state=="ok"){
	        		...
	        		if((type=="img")&&hiddeninput&&res.data){
	        			processImgUploadCallback(res,hiddeninput,isMultiple,imgbox);
...
var FileUploadUtil={
		...
		initFileBoxUI:function(fileBoxs){
			...
			var box,placeholder,tmpplaceholder,width,height,accept,newAccepts,multi,buttonId,inputId,ranId;
			for(var i=0;i<len;i++){
				placeholder="选择文件";
				newAccepts="";
				width=0;
		...
		initFileBoxDatas:function(box){
			...
			if(typeof(filesJsonStr)=="string"){
				if(filesJsonStr.indexOf("[")==-1 && filesJsonStr.indexOf(":")==-1){
					...
					for(var i =0;i<jsonLength;i++){
						...
						files.push({path:tempFileJson[pathAttr],url:tempFileJson[urlAttr],fileName:tempFileJson[filenameAttr],fileSuffix:getFileExtension(tempFileJson[filenameAttr]),isImg:isImg(tempFileJson[filenameAttr])});
					}
					
				}else if(filesJsonStr.indexOf(",")!=-1 && filesJsonStr.indexOf(":")==-1){
					var farr = filesJsonStr.split(",");
					for(var i in farr){
						files.push({path:farr[i],url:farr[i],fileName:getFileName(farr[i]),fileSuffix:getFileExtension(farr[i]),isImg:isImg(farr[i])});
					}
				}else{
					files.push({path:filesJsonStr,url:filesJsonStr,fileName:getFileName(filesJsonStr),fileSuffix:getFileExtension(filesJsonStr),isImg:isImg(filesJsonStr)});
				}
			...
			if(tplContent){
				...
				$.each(files,function(i,file){
					filebox.append("<li data-filebox='"+fileboxId+"' data-hiddeninput='"+hiddeninputId+"'><a style='"+filestyle+"' target='_blank' data-path='"+file.path+"' href='"+file.url+"'>"+file.fileName+"</a><i title='删除' onclick='removeUploadFileBoxLi(this)' class='fa fa-trash text-danger ml-3'></i></li>").show();
				});
			}
		...
		oneFileBoxProcess:function(box,file){
			...
			if(handler){
				...
				if(handler=="uploadFile"){
					uploadFile("file",url,fileName,file[0].files[0],hiddeninput,fileNameInput,sizeinput,null,null,loading,ucallback);
				}else if(handler=="uploadFileToQiniu"){
					loadJBoltPlugin(['qiniu'], function(){
						uploadFileToQiniu(box,"file",file[0].files[0],hiddeninput,fileNameInput,sizeinput,false,null,loading,ucallback);
					});
					...
					var exe_handler=eval(handler);
					  if(exe_handler&&typeof(exe_handler)=="function"){
						  exe_handler(box,"file",file,fileValue);
					}
				}
		...
		changeFiles:function(uploder,fileInput,fileDatas){
				...
				if(handler){
					...
					if(filebox){
						...
						if(isOk(files)&&(files.length>=limit||(files.length+fileDatas.length>limit))){
		...
		clearIt:function(box){
			var inputId=box.find("button").data("inputid");
			var fileInput=jboltBody.find("input[type='file']#"+inputId);
			fileInput.val("");
			box.find("span.j_file_name").text("");
			box.find("div.j_upload_file_box_msg").removeClass("show");
...
var Ajax={
		...
		uploadFormData:function(url,formData,success,error,sync,timeout){
		  ...
		  post:function(url,data,success,error,sync,timeout){
			    var async=true;
			    if(sync){async=false;}
			    url=actionUrl(url);
			...
			get:function(url,success,error,sync,timeout){
				var async=true;
			    if(sync){async=false;}
			    url=actionUrl(url);
				$.ajax({
		...
		readAndInsertItems:function(_thisSelect,setting,refreshing){
				   var that=this;
		      		...
		      		if(_thisSelect.data("text")){
		      			...
		      			_thisSelect.append('<option value="'+firstValue+'">'+_thisSelect.data("text")+'</option>');
		      		}
		      		var url=null;
		 ...
		 exeFun:function(action,url){
			  ...
			  if(handler){
				  ...
				  if(handler=="removeTr"){
					  ...
					  var portalId=action.data("portal")||action.data("portalid");
					  if(portalId){
						  if(portalId == "parentPortal"){
							  if(action){
								  var parentPortal= action.closest("[data-ajaxportal]");
								  if(isOk(parentPortal)){
									  parentPortal.ajaxPortal(true);
								  }
							  }
...
var OpenPageBtnUtil ={
		...
		initBy:function(cssSelector,parentEle){
			 ...
			 parent.on("click",cssSelector,function(e){
				  ...
				  if(!openpage || (openpage&&openpage!='dialog')){
					  var checkHandler=action.data("check-handler");
					  if(checkHandler){
						  var checkResult;
						  var exeCheck_handler=eval(checkHandler);
...
//						 if(isWithtabs()){
						 if(target&&target=="parent"){
							 if(parent.jboltWithTabs){
								 parent.JBoltTabUtil.currentTabGo(action.attr("href")||action.data("url"));
							 }else{
								 parent.JBoltPjaxUtil.openByType(action);
							 }
						 }else{
							 if(jboltWithTabs){
								 JBoltTabUtil.currentTabGo(action.attr("href")||action.data("url"));
							 }else{
...
//	if(self!=top||typeof(JBolt_Left_Nav_width)=="undefined" || !JBolt_Left_Nav_width){return false;}
	...
	if(value=="true"){
		...
		jboltAdmin.find(".jbolt_tabbar").css("left","60px");
		jboltAdmin.find(".jbolt_admin_main").css("left","60px");
		...
		jboltAdmin.find(".jbolt_tabbar").css("left",JBolt_Left_Nav_width+"px");
		jboltAdmin.find(".jbolt_admin_main").css("left",JBolt_Left_Nav_width+"px");
	}
...
//				jboltBody.pjax('a[data-pjax]:not([target]),[data-pjax] a[data-open-type="1"]:not([target])', mainPjaxContainerId);
		...
		afterPjax:function(){
			...
			RangerSliderUtil.init(mainPjaxContainer);
...
function checkExistLayerDialogBtn(title,cssClass){
	 var sameClassBtns=parent.$(".layui-layer-btn").find("a."+cssClass.replace(" ","."));
	 var existBtn=null;
...
function addParentLayerDialogBtn(title,cssClass,clickFunc){
	...
	parent.$("body").on("click","#"+btnId,function(e){
		e.preventDefault();
		e.stopPropagation();
		if(clickFunc){
			clickFunc();
...
function tofixed(number,count,removeZero){
	number=number?Number(number):0;
	count=count||0;
	var value=number?number.toFixed(count):0;
	if(removeZero){
		value=removeFixedNumberZero(value,count);
	}
	return value;
}

function removeFixedNumberZero(value,toFixed){
	if(toFixed){
		var removeStr='.';
...
function jboltHiprintToPdfByHtml(ele,fileName){
	...
	LayerMsgBox.loading("执行中...",20000);
	fileName = (fileName?fileName:("导出PDF_"+new Date().Format("yyyy_MM_dd_HH_mm_ss")));
	var printTemplate = renderBox.data("printtemplate");
	if(printTemplate){
		var printData = renderBox.data("printdata");
		if(printData){
